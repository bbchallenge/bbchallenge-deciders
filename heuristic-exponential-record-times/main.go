...
func heuristicExponentialPassageTimes(tm bbc.TM, timeLimit int, recordLimit int, nbPointsToConclude int, maxA int, maxk int, debug bool) bool {
		...
		go func(iWorker int, nWorkers int) {
			...
			if indexFileName == "" {
				for n := minIndex + iWorker; n < maxIndex; n += nWorkers {
					...
					if heuristicExponentialPassageTimes(m, timeLimit, recordLimit, nbPointsToConclude, maxA, maxk, false) {
						var arr [4]byte

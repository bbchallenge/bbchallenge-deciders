...
impl FromStr for TapeHead {
    type Err = TMError;
    /// Converts strings such as `A>` to TapeHead.
    ///
    /// ```
    /// use std::str::FromStr;
    /// use decider_bouncers_reproduction::directional_tm::{TapeHead, Direction};
    /// assert_eq!(TapeHead::from_str("A>"), Ok(TapeHead { state: 0, pointing_direction: Direction::RIGHT }));
    /// assert_eq!(TapeHead::from_str("<E"), Ok(TapeHead { state: 4, pointing_direction: Direction::LEFT }));
    /// ```
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if s.chars().nth(0).unwrap() == '<' {
            Ok(TapeHead {
                state: (s.chars().nth(1).unwrap() as u8) - b'A',
                pointing_direction: Direction::LEFT,
            })
        } else if s.chars().nth(1).unwrap() == '>' {
            Ok(TapeHead {
                state: (s.chars().nth(0).unwrap() as u8) - b'A',
                pointing_direction: Direction::RIGHT,
            })
        } else {
...
impl Tape {
    pub fn set_machine_str(&mut self, machine_std_format: &str) {
        self.machine_transition = TMTransitionTable::new(machine_std_format);
    }

...
pub struct PotentialFormulaTape {
    pub record_breaking_tapes: Vec<Tape>,
    pub tested_tape_length: HashSet<usize>,
    pub current_index_in_tapes_length: usize,
    pub tested_len1_len2_pairs: HashSet<(usize, usize)>,
    pub tested_len_diff_and_step_diff2: Vec<(usize, i32)>,
}

impl Iterator for PotentialFormulaTape {
    type Item = (FormulaTape, usize);

    /// Returns the next formula tape fitting a trlipe of tapes
    fn next(&mut self) -> Option<Self::Item> {
        if self.record_breaking_tapes.len() < 4 {
            return None;
        }

        let tapes_length = self
            .record_breaking_tapes
            .iter()
            .rev()
            .map(|tape| tape.len())
            .skip(self.current_index_in_tapes_length);

        for len1 in tapes_length {
            let tape1_index = self
                .record_breaking_tapes
                .binary_search_by_key(&len1, |tape| tape.len())
                .unwrap();
            let tape1 = &self.record_breaking_tapes[tape1_index];

            for len2 in self
                .record_breaking_tapes
                .iter()
                .rev()
                .skip(self.record_breaking_tapes.len() - tape1_index)
                .map(|tape| tape.len())
            {
                if self.tested_len1_len2_pairs.contains(&(len1, len2)) {
                    continue;
                }

                self.tested_len1_len2_pairs.insert((len1, len2));

                let tape2_index = self
                    .record_breaking_tapes
                    .binary_search_by_key(&len2, |tape| tape.len())
                    .unwrap();
                let tape2 = &self.record_breaking_tapes[tape2_index];

                let len_diff = len1 - len2;
                //println!("Index: {} {}", tape1_index, tape2_index);
                //println!("LEN: {} {} {}", len1, len2, len_diff);

                // No need to test prefix of already tested sequence
                if self.tested_tape_length.contains(&(len1 + len_diff)) {
                    continue;
                }
                ...
                let tape3: &Tape = match self
                    .record_breaking_tapes
                    .binary_search_by_key(&len3, |tape| tape.len())
                {
                    Ok(index) => &self.record_breaking_tapes[index],
                    Err(_) => continue,
                };
                ...
                for (other_len_diff, other_step_diff2) in self.tested_len_diff_and_step_diff2.iter()
                {
                    if len_diff % other_len_diff == 0
                        && step_diff2
                            == (other_step_diff2
                                * (i32::pow((len_diff / other_len_diff) as i32, 2)))
                    {
                ...
                self.tested_len_diff_and_step_diff2
                    .push((len_diff, step_diff2));

                match self
                    .record_breaking_tapes
                    .binary_search_by_key(&step4, |tape| tape.step_count)
                {
                    Ok(tape4_index) => {
                        let tape4 = &self.record_breaking_tapes[tape4_index];
                        if tape3.len() <= tape4.len() || tape3.len() - tape4.len() != len_diff {
                            continue;
                        }

                        match fit_formula_tape_from_triple(
                            tape3.clone(),
                            tape2.clone(),
                            tape1.clone(),
                        ) {
            ...
            self.tested_tape_length.insert(len1);
            self.current_index_in_tapes_length += 1;
        }
...
pub fn guess_formula_tapes_given_record_breaking_tapes(
    record_breaking_tapes: &Vec<Tape>,
) -> Vec<(FormulaTape, usize)> {
    // for tape in record_breaking_tapes.iter() {
    //     println!("{} {} {}", tape, tape.len(), tape.step_count);
    // }
    ...
    let mut to_return: Vec<(FormulaTape, usize)> = vec![];
            ...
            .map(|tape| tape.len())
        {
            ...
            match record_breaking_tapes.binary_search_by_key(&step4, |tape| tape.step_count) {
                Ok(tape4_index) => {
                    ...
                    {
                        Some(formula_tape) => {
                            to_return.push((formula_tape, tape3.step_count as usize));
                        }
                        None => continue,
                    }
                }
